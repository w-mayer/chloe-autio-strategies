...
This document describes the comprehensive CSS animation system implemented for service cards across the Chloe Autio Strategies website.

## Overview

The "Confident Flow" animation style creates a sophisticated, engaging user experience with smooth, wave-like animations that convey professionalism and trust.
...
### 2. Idle State
- **Continuous Float**: `translateY` oscillates between -3px and 3px over 4 seconds
- **Gentle Rotation**: `rotate` between -0.5deg and 0.5deg
- **Timing**: Sine wave for natural breathing motion

### 3. Hover State
- **Lift Effect**: `translateY(-12px)` and `scale(1.03)` in 0.4s
- **Sophisticated Shadow**: `0 20px 40px rgba(0,0,0,0.15)`
- **Accent Color Flow**: Flows from left to right across card border (2s animation)
- **Icon/Number Pulse**: Subtle pulse on any icons or numbers inside the card
...
- `.service-card-idle` - Continuous floating animation
- `.service-card-ripple` - Ripple effect on click

### Utility Classes
- `.service-card-icon` - Applies pulse animation to icons
- `.service-card-number` - Applies pulse animation to numbers/statistics

## Usage Examples

### Basic Service Card
```jsx
<div className="service-card service-card-entrance service-card-idle">
...
function ServiceCard({ service, index }) {
  ...
  return (
    <div
      className="service-card service-card-entrance service-card-idle"
...
- `animate-service-card-float` - Floating animation
- `animate-service-card-wake` - Wake effect
- `animate-service-card-accent-flow` - Accent flow animation
- `animate-service-card-pulse` - Pulse animation
