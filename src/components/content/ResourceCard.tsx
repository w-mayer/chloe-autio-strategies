...
function ResourceCardInner({ resource }: { resource: Resource }) {
  ...
  return (
    ...
    >
      <div className="mb-2 text-xs text-primary-700">{new Date(resource.date).toLocaleDateString()}</div>
      <h3 className="text-lg font-semibold text-primary-800 mb-2">{resource.title}</h3>
      <p className="text-neutral-700 dark:text-neutral-200 mb-4 flex-1">{resource.summary}</p>
      <div className="flex items-center gap-2 mb-2">
        <Image src={resource.author.avatarUrl} alt={resource.author.name} width={32} height={32} className="w-8 h-8 rounded-full object-cover" />
        <span className="text-sm text-neutral-700 dark:text-neutral-300">{resource.author.name}</span>
      </div>
